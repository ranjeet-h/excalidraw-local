...
- [x] Replace the current template screen in `src/App.tsx` with a document workspace.
- [x] Build a full-height desktop shell with top bar, tab strip, editor canvas, and status bar.
- [x] Add a welcome state for first launch and no-open-document scenarios.
- [ ] Make the shell responsive, dark-mode aware, and keyboard accessible.
- [x] Create a clean product name, title, and UI copy for the final app.
- [x] Remove or quarantine placeholder template components that are no longer needed.
...
- [x] Install and wire `@excalidraw/excalidraw` into the React app.
- [x] Load the editor inside a dedicated workspace component with proper sizing and styling.
- [x] Connect editor `onChange` events to app state so the current tab always knows if it is dirty.
- [x] Preserve Excalidraw app state such as zoom, selected tool, view mode, and theme per tab.
- [ ] Verify undo, redo, clipboard, zoom, pan, keyboard shortcuts, and text editing work inside Tauri.
- [x] Add any peer dependencies or CSS adjustments required by the official package.
- [x] Define a safe wrapper around the editor so file logic stays outside the canvas component.
...
- [x] Design a document schema with `id`, `title`, `filePath`, `elements`, `appState`, `files`, `version`, `dirty`, `lastSavedAt`, and `lastOpenedAt`.
- [x] Add a separate tab state model so each open file can keep its own editor state and save status.
- [x] Create serialization and deserialization helpers for reading and writing Excalidraw files.
- [ ] Add version checks and a migration strategy for future schema changes.
- [x] Make new untitled docs predictable, with stable default names like `Untitled-1`.
- [x] Ensure a failed parse or partial file read does not destroy the current in-memory document.
...
- [x] Implement New Document so users can create a blank tab without losing the current one.
- [x] Implement Open File using a native file picker and support opening one or many files at once.
- [x] Implement Save for tabs that already have a file path.
- [x] Implement Save As to choose a new name and location, then bind the tab to that new path.
- [ ] Add Save Copy As if you want a duplicate file without changing the current tab path.
- [x] Add clear failure handling for permission errors, invalid paths, and disk write errors.
- [ ] Show save progress and success or failure feedback in the UI.
- [x] Prompt before closing a dirty tab, opening a new file over a dirty tab, or quitting the app.
...
- [x] Build a tab manager that can open, close, and activate multiple documents.
- [ ] Add tab reordering if browser-style drag reordering is in scope for v1.
- [x] Keep tab state independent so switching tabs feels instant and no work is lost.
- [ ] Show unsaved indicators, file icons, and loading or saving states on each tab.
- [x] Support middle-click or close-button tab closing if it matches the desktop UX target.
- [x] Add Duplicate Tab or Duplicate Document if it helps users branch ideas quickly.
- [ ] Add reopen-closed-tab behavior if it fits the browser-like model.
- [x] Restore the last session's tabs on app launch, including unsaved recovery tabs.
...
- [x] Store app settings, UI preferences, and recent files in the Tauri app data directory.
- [x] Save recovery snapshots for unsaved tabs so a crash or forced quit does not lose work.
- [x] Restore recovery snapshots on launch and clearly label recovered content.
- [x] Add optional autosave for saved files with a debounce so writes do not happen on every keystroke.
- [x] Keep a lightweight backup or shadow-copy strategy for safer local writes.
- [x] Clean up old recovery files and backups using a predictable retention policy.
- [x] Detect when a file changed on disk outside the app and ask the user how to resolve the conflict.
...
- [x] Add a Mermaid import entry point from menu, toolbar, or command palette.
- [x] Support paste-to-convert for Mermaid text when the user explicitly chooses the action.
- [x] Use `@excalidraw/mermaid-to-excalidraw` to generate elements from Mermaid source.
- [x] Decide whether Mermaid content opens in a new tab or inserts into the current tab by default.
- [x] Provide a preview or confirmation step before replacing existing canvas content.
- [x] Show conversion errors in a human-friendly way with a copyable error-detail block.
- [x] Add sample Mermaid snippets to help users discover the feature.
...
- [x] Support export to PNG for easy sharing.
- [x] Support export to SVG for vector workflows.
- [x] Support export back to native Excalidraw JSON.
...
- [x] Add reveal-in-folder or show-in-finder actions for the active document.
...
- [x] Wire the app to handle OS-level open-file events so double-clicking a `.excalidraw` file opens the right tab.
- [x] Add drag-and-drop open support for files dropped onto the window.
...
- [x] Add a strong empty state with New, Open, and Recent Files actions.
- [x] Add a recent files panel so users can continue quickly.
- [x] Add a command palette or menu structure for power users.
- [x] Add keyboard shortcuts for New, Open, Save, Save As, Close Tab, Switch Tab, Export, and Mermaid Import.
...
- [x] Make the app title, icons, and menu labels match the final product identity.
...
- [x] Build the editor shell and Excalidraw integration.
- [x] Add document serialization and save/open flows.
- [x] Add tabs and session restore.
- [x] Add recovery and autosave.
- [x] Add Mermaid import and export tools.
